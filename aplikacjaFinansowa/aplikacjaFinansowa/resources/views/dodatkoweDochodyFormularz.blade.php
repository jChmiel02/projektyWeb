...
<head>
    ...
    <link href="{{ asset('/CSS/dodatkoweDochodyStyle.css') }}" rel="stylesheet">
...
<div class="container-fluid">
    <div class="row text-center">
        <h1>Tutaj wpisz swoje Dodatkowe dochody
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="infoModalLabel">Informacje o stronie Dodatkowe dochody</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body1">
                    <p>Podstrona 'Dodatkowe dochody' pozwala użytkownikowi wprowadzić typ dochodu, który nie jest
                        cykliczny. Są to na przykład wygrane losowe lub darowizny pieniężne.</p>
                </div>
...
@if(session()->has('user'))
    @php
        $user = session('user');
        $dochody = \App\Models\dodatkowe_dochody::where('uzytkownik_id', $user->getKey())->get();
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-6 mb-3 styled-column">
                <form method="POST" action="{{ route('dodatkoweDochody') }}" id="dochodForm">
                    @csrf
                    <div class="styled-box">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="nazwa" name="nazwa" class="form-control"
                                   placeholder="Podaj nazwę dochodu" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="text" name="kategoria" id="kategoria" class="form-control"
                                   placeholder="Wpisz kategorię dochodu:" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" name="kwota" id="kwota" class="form-control"
                                       placeholder="Kwota dochodu:" required>
                            </div>
                        </div>
                ...
                @if(count($dochody) > 0)
                    @foreach($dochody as $dochod)
                        <div id="editFormContainer" style="display: none;">
                            <h2>Edycja dodatkowego dochodu:</h2>
                            <form method="POST" action="{{ route('updateDodatkoweDochody', ['id' => $dochod->id]) }}"
                                  id="editDochodForm">
                                @csrf
                                @method('PUT')
                                <div class="row">
            ...
            <div class="col-md-6 mb-3 styled-column">
                @if(count($dochody) > 0)
                    <h2>Dodatkowe dochody:</h2>
                    <ul class="list-group">
                        @foreach($dochody as $dochod)
                            <li class="list-group-item dochodItem" data-dochod-id="{{ $dochod->id }}"
                                data-dzien="{{ $dochod->data }}" data-kwota="{{ $dochod->kwota }}"
                                data-kategoria="{{ $dochod->kategoria }}" data-nazwa="{{ $dochod->nazwa }}">
                                <div class="row">
                                    <div class="col">
                                        <h5>Nazwa:</h5>
                                        <p>{{ $dochod->nazwa }}</p>
                                    ...
                                    <div class="col">
                                        <form method="POST"
                                              action="{{ route('deleteDodatkoweDochody', ['id' => $dochod->id]) }}">
                                            @csrf
                                            @method('DELETE')
...
<script src="{{ asset('/JS/dodatkoweDochodyScript.js') }}"></script>
